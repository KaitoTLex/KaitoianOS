...
let
  ...
  arc = if system == "aarch64-linux" then "aarch64-linux" else "x86_64-linux";
in
{
  imports = [
    ./binds.nix
  ...
  config = lib.mkIf cfg.enable {
    ...
    wayland.windowManager.hyprland = {
      enable = true;
      package = inputs.hyprland.packages.${arc}.hyprland;
      portalPackage = inputs.hyprland.packages.${arc}.xdg-desktop-portal-hyprland;
      plugins = [ inputs.hyprscroller.packages.${arc}.hyprscroller ];
      settings = {
        input.touchpad = {
          natural_scroll = true;
